...
**Note:** For Seqera Platform deployment, use the studio git integration. Platform builds the image automatically via Wave using `.seqera/studio-config.yaml`.

## Architecture

### Multi-Stage Docker Build

The Dockerfile (`.seqera/Dockerfile`) uses a 3-stage build to keep the final image lean:
...
| `.seqera/Dockerfile`              | Seqera Platform image (uses connect-client)                    |
...
Verify discovery output inside container:
